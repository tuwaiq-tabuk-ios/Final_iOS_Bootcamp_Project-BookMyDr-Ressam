...
class ViewController: UIViewController {
  ...
  var singUpButton: UIButton = {
    ...
    btn.setTitleColor(.white,
                      for: .normal)
    btn.setTitle("Sign Up".localized(),
                 for: .normal)
    btn.titleLabel?.font = UIFont(name: "American Typewriter",
                                  size:31 )
  ...
  var changeLanguegeSegmetedControl: UISegmentedControl = {
    ...
    SegmetedControl.backgroundColor = UIColor
      .init(red: 255/255,
            green: 147/255,
            blue: 0/255,
            alpha: 1)
    
    if let languege = UserDefaults
        .standard
        .string(forKey: "currentLanguage"){
      switch languege {
      case "en":
      ...
      let localLanguege = Locale.current.languageCode
      UserDefaults.standard.setValue([localLanguege],
                                     forKey: "AppleLanguage")
      if localLanguege == "ar" {
        SegmetedControl.selectedSegmentIndex = 0
      } else{
        SegmetedControl.selectedSegmentIndex = 1
  ...
  var label:UILabel = {
    ...
    label.font = UIFont(name: "Hoefler Text Italic"
                        , size: 31.0)
  ...
  override func viewDidLoad() {
    ...
    changeLanguegeSegmetedControl.frame =
    ...
    singUpButton.addTarget(self,
                           action: #selector(singUpButton(_:)),
                           for:.touchUpInside)
    loginButton.addTarget(self,
                          action: #selector(btnloginButton(_:)),
                          for:.touchUpInside)
    
    changeLanguegeSegmetedControl.addTarget(self,
